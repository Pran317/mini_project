...
if __name__ == "__main__":
    ...
    pool_vectors = torch.max(output[0], dim=1).values      # Fill up this '----------' section
    # cls_vectors (2x768) is the vector generated by taking the CLS token
    cls_vectors = output[0][:, 0, :]       # Fill up this '----------' section
    
    # Compute the cosine similarity
    cosine_pooling = torch.nn.functional.cosine_similarity(pool_vectors[0].unsqueeze(0), pool_vectors[1].unsqueeze(0), dim=1)       # Fill up this '----------' section
    cosine_cls = torch.nn.functional.cosine_similarity(cls_vectors[0].unsqueeze(0), cls_vectors[1].unsqueeze(0), dim=1)      # Fill up this '----------' section
